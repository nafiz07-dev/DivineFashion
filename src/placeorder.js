...
let totalAmount = hash.includes('Discount') ? totalAmnt.split('=')[1] : totalAmnt.slice(2);
// let totalAmount = totalAmnt.slice(2);

// if (hash.includes('discount')) {
const discountAmount = (hash.includes('Discount')) ? discountAndAmount.split('?')[0].split('=')[1] : '';
...
const loadProductArrFnl =
  ...
  !hash.includes('$')
    ? loadProductArr
    : loadProductArr.filter((i) => !availableSize.includes(i));

// const loadProductArrFnl = loadProductArr.filter((i) =>
//   !availableSize.includes(i)
// );

// console.log(loadProductArrFnl);

// console.log(availableSize.includes('XL'));
...
const sendOrder = function () {
  let parms = {
    ...
    discount_applied_in_order_page:
    ...
    customer_email: vers.placeOrderEmail.value,
